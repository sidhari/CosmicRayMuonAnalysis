...
int corsika_output_file_reader::process_run_header_sub_block(vector<float> run_header_sub_block,TTree* RUNH_tree)
{
...
int corsika_output_file_reader::process_event_header_sub_block(vector<float> event_header_sub_block, TTree* EVTH_tree)
{
...
bool corsika_output_file_reader::is_mother_particle_data_line(float particle_id,unsigned int ancestor_count)
{
    if(particle_id < 0 && ancestor_count == 0)
        return true;
    else
        return false;
}

bool corsika_output_file_reader::is_grandmother_particle_data_line(float particle_id,unsigned int ancestor_count)
{
    if(particle_id < 0 && ancestor_count == 1)
        return true;
    else
        return false;
}
...
int corsika_output_file_reader::process_input_file(string input_filepath,TTree* RUNH_tree,TTree* EVTH_tree,TTree* PD_tree)
{
    ifstream input_file;
    input_file.open(input_filepath,ios::in); //open input file for processing
    ...
    {   
        ...
        {   
            ...
            if(process_run_header_sub_block(run_header_sub_block,RUNH_tree))
            {
        ...
        { 
            ...
            if(process_event_header_sub_block(event_header_sub_block,EVTH_tree))
            {
        ...
        {   
            ...
            else if(is_mother_particle_data_line((float)particle_id,ancestor_count))
            {
            ...
            else if(is_grandmother_particle_data_line((float)particle_id,ancestor_count))
            {
